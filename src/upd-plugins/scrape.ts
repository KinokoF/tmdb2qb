...
export async function scrapeOfficialPlugins(): Promise<ProtoPlugin[]> {
  ...
  return $("div.markdown-body table:first-of-type tr:not(:first-of-type)")
    .map((_, e) => {
      const lastUpdate = $(e).find("td:nth-child(3)").text();
      const download = $(e).find("td:nth-child(4) a").attr("href")!;
      const comments = $(e).find("td:nth-child(5)").text();

      const url = new URL(download);
      const name = url.pathname.split("/").pop()!;
      const updated = moment(lastUpdate, "DD/MMM/YYYY").unix();
...
export async function scrapeLightDestoryPlugins(): Promise<ProtoPlugin[]> {
  ...
  return $("article.markdown-body table tbody tr")
    .map((_, e) => {
      const rawUpdated = $(e).find("td:nth-child(2) em").text();
      const rawWorking = $(e).find("td:nth-child(3)").text();
      const download = $(e).find("td:nth-child(4) a").attr("href")!;
